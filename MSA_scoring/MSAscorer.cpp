...
int main(int argc, char * argv[]) {
    ...
    int difference = refData->at(0).length() - testData->at(0).length();
    //cout << "Before adjustment: Test MSA length: " << testData->at(0).length() << ", Ref MSA length: " << refData->at(0).length() << endl; // Diagnostic print

    for(size_t i = 0; i < testData->size(); ++i) {
    string updatedSequence = testData->at(i).Seq() + string(difference, '-');
    CSequence newSequence(testData->at(i).Name(), updatedSequence); // Assuming CSequence has a constructor that takes a name and a sequence.
    (*testData)[i] = newSequence;
}


    //cout << "After adjustment: Test MSA length: " << testData->at(0).length() << ", Ref MSA length: " << refData->at(0).length() << endl; // Diagnostic print

    if(testData->at(0).length() != refData->at(0).length()) {
        cout << "\nError: Failed to adjust test MSA length to match reference MSA\n";
        exit(-1);
    }
}
